...
func start_match_host() -> void:
	if multiplayer.is_server():
		_start_match_local()
		rpc("_start_match_remote")

func on_start_button_pressed() -> void:
	if multiplayer.is_server():
		# บอกทุก peer รวมถึงตัวโฮสต์เองให้เริ่ม (call_local)
		_start_match_local()
		rpc("_start_match_remote")   # broadcast ไปทุก client
...
func _rebuild_turn_order() -> void:
	...
	for s in sides:
		var pid: int = players_peer_map.get(s, 0)   # ต้องมี players_peer_map ประกาศไว้ด้านบนไฟล์
		if pid != 0 and not _turn_order.has(pid):
			_turn_order.append(pid)
	if _turn_order.is_empty():
		_turn_order = [1]  # กันล่ม
	_turn_index = 0
...
func _start_match_local() -> void:
	# TODO: reset UI, แจกไพ่, สุ่มลำดับ ฯลฯ ตามเกมคุณ
	if multiplayer.is_server():
		_start_server_side_rules()
	
# สมมุติว่าคุณมีรายชื่อผู้เล่น 4 ฝ่าย: Good, Call, Hacker, Police
# และเก็บ mapping ว่า peer_id ไหนคือผู้เล่นไหน
...
func assign_piece_authority() -> void:
	...
	for piece in $Pieces.get_children():
		var peer_id := resolve_owner_peer_for_piece(piece)
		if peer_id == 0:
...
func _kick_unassigned_pieces() -> void:
	for piece in $Pieces.get_children():
		...
		if owner == 0 or (owner == 1 and not _is_npc_allowed(piece)):
			_remove_piece_server(piece)

func _is_npc_allowed(piece: Node) -> bool:
	# ถ้ามี NPC ให้ตั้งกฎ/แท็กเอง เช่นชื่อขึ้นต้นด้วย "NPC_"
...
# ===== Multiplayer state =====
var players_peer_map: Dictionary = {}   # จะถูกเติมค่าใน _assign_players_to_sides()





func resolve_owner_peer_for_piece(piece: Node) -> int:
	var n := piece.name
	if n.begins_with("Good"):
		return players_peer_map.get("Good", 0)
	if n.begins_with("Call"):
		return players_peer_map.get("Call", 0)
	if n.begins_with("Hacker"):
		return players_peer_map.get("Hacker", 0)
	if n.begins_with("Police"):
		return players_peer_map.get("Police", 0)
	return 0
	
	# ถ้าใช้วิธีอื่น เช่นมี field/metadata บน node ให้ดึงแทน
	# return piece.get("owner_peer_id") if piece.has_method/prop ดังกล่าว
...
func _move_piece_visual_only(piece: Node, target_cell: Vector2i) -> void:
	...
	if piece is Node2D:
		piece.global_position = _grid_to_world(target_cell)

func _grid_to_world(cell: Vector2i) -> Vector2:
	# ปรับตาม CELL_SIZE/ออฟเซ็ตของโปรเจกต์คุณ
...
func _assign_players_to_sides() -> void:
	var host_id := 1  # ENet: host คือ 1 เสมอ
	var peers: Array = multiplayer.get_peers() # รายชื่อ peer id ของ client ที่ต่ออยู่ (ไม่รวม host)
	var sides := ["Good", "Call", "Hacker", "Police"]
	...
	# ฝ่ายที่เหลือไม่มีผู้เล่น → ให้ 0 (ไม่มีเจ้าของ)
...
func _side_from_piece_name(n: String) -> String:
	if n.begins_with("Good"): return "Good"
	if n.begins_with("Call"): return "Call"
	if n.begins_with("Hacker"): return "Hacker"
	if n.begins_with("Police"): return "Police"
	return ""
...
func add_card_to_hand(card, side: String) -> void:
	# กัน “อัปเดตผิดมือ”: ถ้ามือที่กำลังแสดงไม่ใช่ของฝั่งนี้ ให้หยุดเลย
	if side != _local_side():      # single-player ใส่ให้คืน "Good" หรือฝั่งที่ผู้เล่นบังคับอยู่
		return
func _local_side() -> String:
	return "Good"  # ถ้าตอนนี้เล่นเดี่ยวเป็น Good เสมอ; ถ้าคุณมีตัวแปรชี้ว่าฝั่งไหนกำลังเล่น ให้คืนค่านั้น
...
func _give_darkweb_cards(p: Sprite2D, count: int) -> void:
	...
	var side := _side_from_piece_name(pieces.name)  # Good/Call/Hacker/Police
	for i in 2:
		var card = _draw_random_card()                    # ใช้ฟังก์ชันเดิมของคุณ
		add_card_to_hand(card, side)               # ← ส่ง side เพิ่มเข้าฟังก์ชันเดิม
